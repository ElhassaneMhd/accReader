...
class MailWizzService {
  ...
  async getAllCampaigns(page = 1, perPage = 10) {
    try {
      ...
      logger.error(
        "MailWizz getAllCampaigns error:",
  ...
  async getCampaignStats(campaignId) {
    try {
      ...
      logger.error(
        "MailWizz getCampaignStats error:",
        error.response?.data || error.message
      );
  ...
  // Subscribers
  async getListSubscribers(listId, page = 1, perPage = 10, search = "") {
    try {
      ...
      logger.error(
        "MailWizz getListSubscribers error:",
        error.response?.data || error.message
      );
  ...
  async addSubscriber(listId, subscriberData) {
    try {
      ...
      logger.info(
        `MailWizz addSubscriber - Data being sent:`,
      ...
      logger.info(
        `MailWizz addSubscriber - Success response:`,
  ...
  async updateSubscriber(listId, subscriberId, subscriberData) {
    try {
      ...
      logger.error(
        "MailWizz updateSubscriber error:",
        error.response?.data || error.message
      );
  ...
  async deleteSubscriber(listId, subscriberId) {
    try {
      ...
      logger.error(
        "MailWizz deleteSubscriber error:",
        error.response?.data || error.message
      );
    ...
    try {
      ...
      for (const endpoint of possibleEndpoints) {
        try {
          ...
          logger.info(
            `Headers being sent: ${JSON.stringify(Object.keys(headers))}`
          );
          ...
          lastError = error;
          logger.warn(
            `Failed with endpoint ${endpoint}: ${error.response?.data?.error || error.message}`
          );
          continue;
        }
      }
      ...
      logger.error(
        "MailWizz importSubscribersCSV error:",
        error.response?.data || error.message
      );
  ...
  async getAllTemplates(page = 1, perPage = 10) {
    try {
      ...
      logger.error(
        "MailWizz getAllTemplates error:",
        error.response?.data || error.message
      );
  ...
  async getAllCustomers(page = 1, perPage = 10) {
    try {
      ...
      logger.error(
        "MailWizz getAllCustomers error:",
        error.response?.data || error.message
      );
  ...
  async getDeliveryServers(page = 1, perPage = 10) {
    try {
      ...
      logger.error(
        "MailWizz getDeliveryServers error:",
        error.response?.data || error.message
      );
